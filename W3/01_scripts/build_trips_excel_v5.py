"""W3 - v5: sorted workbook (Trip ID + Raw Row ID) AND raw-order workbook
(Raw_Drivers_Data.xlsx: Row ID, Trip ID, Row type + untouched originals in file order).
Based on v4: + bin material, fill,
sensor comparison and estimated kg per row, matching Explorer v8).
Based on v3, three assignment tiers,
glass-inclusive v5 base: trips_v5.json + reading_assignments_v5.parquet).
...
tracks = json.load(open(f"{BASE}/W2/02_data_work/trips_v5.json", encoding="utf-8"))
...
# ---- stop enrichment lookup from explorer year files (9-element stops) ----
import glob
stoplut = defaultdict(list)   # (track_id, cid, hhmm) -> [(fill, est, mat, sensor)]
for yf in sorted(glob.glob(f"{BASE}/W3/03_outputs/explorer/data/year_*.json")):
    yd = json.load(open(yf, encoding="utf-8"))
    for tid, stops in yd["stops"].items():
        for st in stops:
            fill = st[5] if len(st) > 5 else None
            est = st[6] if len(st) > 6 else None
            mat = st[7] if len(st) > 7 else None
            snr = st[8] if len(st) > 8 else None
            stoplut[(str(tid), str(st[2]), st[3])].append((fill, est, mat, snr))
...
ra = pd.read_parquet(f"{BASE}/W2/02_data_work/reading_assignments_v5.parquet")
...
widths = [11, 9, 7, 7, 32, 11, 10, 12, 9] + [10, 12, 11, 8, 22, 10, 10, 16, 14, 10, 10, 19, 10, 9, 8, 19, 19, 9, 10]
...
for _, row in in_trip.iterrows():
    ...
    cand = stoplut.get((str(row["_track"]), row["_cid"], hhmm))
    fill = est = mat = snr = None
    if cand:
        fill, est, mat, snr = cand.pop(0) if len(cand) > 1 else cand[0]
    if row["_rowtype"].startswith("pre-reading"):
        fill = est = None   # its own Enchimento column already carries the value
...
binmat = {}
for (tid, cid, hhmm), lst in stoplut.items():
    if lst and lst[0][2]: binmat[cid] = lst[0][2]
COLS2 = COLS + ["Bin material", "Why not assigned"]
for c, h in enumerate(COLS2):
    ws2.write(0, c, h, F["head"])
for c, w in enumerate(widths[8:] + [11, 36]):
...
for _, row in loose.iterrows():
    ...
    ws2.write(r2, len(COLS) + 1, why, F["w"])
    r2 += 1
...
notes = [
    ...
    "",
    "",
    "NEW COLUMNS (v4, matching the online explorer):",
    ...
    "    (trips are 99.97% material-pure).",
    "  · Fill % before emptying — for emptying records: the pre-emptying reading; for",
    "    inferred rows: the reading itself. Blank on pre-reading rows (their Enchimento IS the fill).",
    ...
    "",
    "All 19 original columns are unchanged and in their original order after the 8 helper columns.",
...
# ---- RAW-ORDER workbook ----
RAWOUT = f"{BASE}/W3/03_outputs/tables/Raw_Drivers_Data.xlsx"
...
for c, h in enumerate(RH + COLS):
    wsr.write(0, c, h, Fh)
for c, w in enumerate([8, 11, 32] + widths[9:]):
...
for _, row in raw.iterrows():
    ...
    if row["_track"] is not None and row["_track"] == row["_track"]:
        ...
        alt = row["_alt"]
        tid, typ = "", (f"ambiguous between {row['_cand']} and {alt}" if alt and alt == alt else "ambiguous")
    else:
        tid, typ = "", {"NO_TRIP_RUNNING": "no trip running", "INFEASIBLE": "infeasible"}.get(st, "")
    fmt = Fw if typ == "emptying record" else Fi
...
for i, n in enumerate([
    ...
    "    plus a track letter when one number hid several vehicles (e.g. 17242b).",
    "    Blank = the row could not be safely assigned to any trip.",
    "  · Row type — how the row relates to its trip: emptying record (has idrecolha),",
    "    pre-reading (fill noted ≤15 min before that emptying), inferred (assigned by time +",
    "    location), or the reason it stays unassigned (ambiguous / no trip running / infeasible).",
